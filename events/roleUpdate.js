...
module.exports = (client, oldRole, newRole) => {
  db.findOne(
    ...
    (err, data) => {
      ...
      const modLogChannel = newRole.guild.channels.find(
      ...
      else logEmbed.addField("Name (not changed)", oldRole.name);

      if (oldRole.permissions !== newRole.permissions)
        logEmbed.addField(
          "Permissions (changed)",
          "~~[Old Permissions](https://discordapi.com/permissions.html#" +
            oldRole.permissions +
            ")~~\n" +
        ...
        logEmbed.addField(
          ...
          "[New Permissions](https://discordapi.com/permissions.html#" +
            oldRole.permissions +
            ")"
        );
      ...
      else logEmbed.addField("Mentionable (not changed)", oldRole.mentionable);

      if (oldRole.hoist !== newRole.hoist)
        logEmbed.addField("Users Displayed Seperate (changed)", newRole.hoist);
      else
        logEmbed.addField(
          "Users Displayed Seperate (not changed)",
          newRole.hoist
        );
